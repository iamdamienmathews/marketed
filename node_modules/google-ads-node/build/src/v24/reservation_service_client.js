 ...
 * `src/v24/reservation_service_client_config.json`.
...
class ReservationServiceClient {
    ...
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.ads.googleads.v24.services.ReservationService', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        this.reservationServiceStub = this._gaxGrpc.createStub(this._opts.fallback
            ? this._protos.lookupService('google.ads.googleads.v24.services.ReservationService')
        ...
        const reservationServiceStubMethods = ['quoteCampaigns', 'bookCampaigns'];
    ...
    quoteCampaigns(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        ...
        this._log.info('quoteCampaigns request %j', request);
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('quoteCampaigns response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls
            .quoteCampaigns(request, options, wrappedCallback)
    ...
    bookCampaigns(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        this._log.info('bookCampaigns request %j', request);
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('bookCampaigns response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls
            .bookCampaigns(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('bookCampaigns response %j', response);
            return [response, options, rawResponse];
        })
